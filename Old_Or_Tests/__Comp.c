...
void set_pixel_succesors(Pixel_Node_t *current, Color_t Region_Color,
  ...
  if (!is_in_Graphe(current->pixel)) {
    ...
    for (size_t i = 0; i < 4; i++) {
      Pixel_Node_t *to_add = malloc(sizeof(Pixel_Node_t));
    ...
    for (size_t i = 0; i < current->nexts->size; i++) {
      Pixel_Node_t *tmp = see_elem(current->nexts, i);
      if (!is_in_Graphe(tmp->pixel)) {
        pushfront_elem_no_cpy(&Stack, tmp);
      }
    }
  }
}
...
void get_region_bords(Region_t *to_proccess) {
  ...
  while (Stack.size) {
    ...
    if (Seen_Bords[current_node->pixel] == FALSE) {
      Seen_Bords[current_node->pixel] = TRUE;
      ...
      for (size_t i = 0; i < current_node->nexts->size; i++) {
        Pixel_Node_t *tmp = see_elem(current_node->nexts, i);
        if (Seen_Bords[tmp->pixel] == FALSE) {
          pushfront_elem_no_cpy(&Stack, tmp);
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  Seen_Bords = malloc(sizeof(Pixel_Node_t) * image->image_size);
